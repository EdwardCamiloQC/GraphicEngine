...
MapCube::MapCube(float xCam, float yCam, float zCam):Map(xCam, yCam, zCam){
    cube = Figure::generateCube();
    position.x = 0.0f;
    position.y = 0.0f;
    position.z = 0.0f;
...
void MapCube::implementMap(unsigned int idShader){
    ...
    glBindVertexArray(VAO);
    glDrawElements(GL_TRIANGLES, sizeof(cube.indices)/sizeof(float), GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
}
